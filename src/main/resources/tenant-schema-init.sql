-- ══════════════════════════════════════════════════════════════════════════════
-- tenant-schema-init.sql — Schéma COMPLET et IDEMPOTENT pour chaque base tenant
-- Contient TOUTES les tables en CREATE TABLE IF NOT EXISTS → 100% safe
-- Utilisé par :
--   - initialiserTablesTenant()      : lors de la création de chaque entreprise
--   - normaliserCredentialsTenants() : au démarrage pour réparer les bases existantes
--
-- NE PAS mettre de ALTER TABLE ici : ils échouent sur ScriptUtils si la colonne
-- existe déjà (bases créées avec CREATE TABLE déjà complet)
-- ══════════════════════════════════════════════════════════════════════════════
...
-- ─────────────────────────────────────────────────────────────
-- TABLE : roles_config
-- Stocke la matrice complète des rôles et permissions en JSON.
-- Un seul enregistrement par tenant (id=1, upsert via INSERT...ON DUPLICATE KEY UPDATE).
-- ─────────────────────────────────────────────────────────────
...
-- Colonnes alignées avec LigneCommandeAchat.java
-- ─────────────────────────────────────────────────────────────
